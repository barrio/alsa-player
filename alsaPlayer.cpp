...
bool read_wav_header(const std::string &filename, WavHeader &header,
  ...
  // Use std::string_view for tag comparisons
  const std::string_view riff_tag(header.riff_tag, 4);
  const std::string_view wave_tag(header.wave_tag, 4);
...
void configure_alsa(snd_pcm_t *&pcm_handle, const WavHeader &header) {
  ...
  if (snd_pcm_hw_params_set_channels(pcm_handle, params, header.num_channels) <
      0) {
    throw std::runtime_error("Failed to set ALSA channels.");
  }

  if (snd_pcm_hw_params_set_rate(pcm_handle, params, header.sample_rate, 0) <
      0) {
...
void play_audio(snd_pcm_t *pcm_handle, const std::vector<uint8_t> &data,
                size_t frame_size) {
  size_t total_frames = data.size() / frame_size;

  for (size_t i = 0; i < total_frames;) {
    snd_pcm_sframes_t frames_written = snd_pcm_writei(
        pcm_handle, data.data() + i * frame_size, total_frames - i);
    if (frames_written < 0) {
...
int main(const int argc, char* argv[]) {
  ...
  try {
    ...
    std::cout << "Bit Depth: " << header.bits_per_sample << " bits"
              << std::endl;
